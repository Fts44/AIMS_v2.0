                ...
                <table id="table" class="table table-bordered" style="width: 100%;">
                    ...
                    @foreach($ie_brands as $item)
                        <tr>
                            ...
                            @else
                                <td>
                                    <a class="btn btn-primary btn-sm" onclick="update_form('{{ $item->ieb_id }}','{{ $item->ieb_brand }}','{{ $item->ieb_status }}')"><i class="bi bi-pencil"></i></a>
                                    <button class="btn btn-danger btn-sm" {{ ($item->ieid_id!=null) ? 'disabled' : '' }} onclick="delete_form('{{ $item->ieb_id }}','{{ $item->ieb_brand }}');"><i class="bi bi-eraser"></i></button>
                                </td>
                            @endif
                        </tr>
                    @endforeach
...
@push('script')
    ...
    <script>    
        ...
        function clear_form(){
            ...
            $('#brand').val('');
            $('#lbl_form').html('Add');
            $('#form').attr('action', "{{ route('Admin.Equipment.Brand.Insert') }}");
        }

        function update_form(item_id, item_place, item_status){
            clear_form();
            var url = "{{ route('Admin.Equipment.Brand.Update', ['id'=>'%id%']) }}";
            $('#form').attr('action', url.replace('%id%', item_id));
            $('#brand').val(item_place);
        ...
        function delete_form(id, place){
            var url = "{{ route('Admin.Equipment.Brand.Delete', ['id'=>'%id%']) }}";
